...
class BenchmarkConfig:
    ...
    # None = don't save, "largest" = save largest hashtable size, "smallest" = save smallest hashtable size
...
def run_benchmark(
        ...
        if dataset.batch_size is None:  # only calculates once in repeat loop
            if cfg.batch_size is None:
                ...
                dataset.batch_size = calculate_batch_size(
                    ...
                    optimizer=optimizer,  # Pass optimizer to calculate batch size
                )
                ...
                if is_main_process:
                    print(
                        f"Calculated batch size: {dataset.batch_size:,} per GPU for training"
                    )
            else:
        ...
        if is_main_process:
            ...
            with torch.no_grad():
                ...
                for i in tqdm(range(num_slices), disable=not cfg.enable_pbar):
                    gt_slice = (
                        gt_data[:, :, i].unsqueeze(0).unsqueeze(0)
                    )  # (1, 1, H, W)
    ...
    if is_main_process:
        ...
        if should_save:
            ...
            # Use eval_dataset if available for restoration
            restore_dataset = eval_dataset if "eval_dataset" in locals() else dataset
            reconst_bytes = (
                # restore() denormalizes and unshuffles the data
                restore_dataset.restore(reconst_data.flatten())
                .cpu()
                .numpy()
                .astype(save_type)
                .tobytes(save_order)
...
def calculate_batch_size(
    ...
    # Use 99% of working batch for safety margin
    final_batch = max(1, int(best_batch * 0.99))
